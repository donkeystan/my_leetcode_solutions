...
{
    struct ListNode* p_chk = NULL;
    struct ListNode* p_cur = NULL;
    struct ListNode* p_pin = NULL;
    ...
    while (NULL != p_chk)
    {
        p_chk = (struct ListNode *)p_chk->next;

        if ((NULL != p_chk) && (NULL != p_cur))
        {
            if (p_chk->val != p_cur->val)
            {
                p_pin = p_cur;
                p_cur = (struct ListNode *)p_cur->next;
            }
            else
            {
                while (NULL != p_chk)
